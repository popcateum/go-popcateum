...
// the near future.
func (lru *lru) get(epoch uint64, epochLength uint64, bubblepopBlock *uint64) (item, future interface{}) {
	lru.mu.Lock()
	defer lru.mu.Unlock()
	...
	if bubblepopBlock != nil {
		nextEpochBlock := nextEpoch * epochLength
		if nextEpochBlock == *bubblepopBlock && epochLength == epochLengthDefault {
			nextEpoch = nextEpoch / 2
			nextEpochLength = epochLengthBubblePop
		}
	}
...
type cache struct {
	...
	epochLength uint64    // Epoch length (BubblePop)
...
func (ethash *Ethash) cache(block uint64) *cache {
	epochLength := calcEpochLength(block, ethash.config.BubblePopBlock)
	epoch := calcEpoch(block, epochLength)
	currentI, futureI := ethash.caches.get(epoch, epochLength, ethash.config.BubblePopBlock)
...
func (ethash *Ethash) dataset(block uint64, async bool) *dataset {
	...
	epochLength := calcEpochLength(block, ethash.config.BubblePopBlock)
	epoch := calcEpoch(block, epochLength)
	currentI, futureI := ethash.datasets.get(epoch, epochLength, ethash.config.BubblePopBlock)
...
// CalcEpochLength returns the epoch length for a given block number (BubblePop)
func CalcEpochLength(block uint64, bubblepopBlock *uint64) uint64 {
	return calcEpochLength(block, bubblepopBlock)
}

// CalcEpoch returns the epoch for a given block number (BubblePop)
func CalcEpoch(block uint64, epochLength uint64) uint64 {
	return calcEpoch(block, epochLength)
}

func (ethash *Ethash) GetBubblePopBlockNumber() *uint64 {
