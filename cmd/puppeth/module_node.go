...
// nodeDockerfile is the Dockerfile required to run an Popcateum node.
var nodeDockerfile = `
FROM popcateum/client-go:latest
...
{{end}}
RUN \
  echo 'gpop --cache 512 init /genesis.json' > gpop.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.popcateum/keystore/ && cp /signer.json /root/.popcateum/keystore/' >> gpop.sh && \{{end}}
	echo $'exec gpop --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Popcatbase}}--miner.popcatbase {{.Popcatbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gpop.sh

ENTRYPOINT ["/bin/sh", "gpop.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Popcateum node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.popcateum{{if .Ethashdir}}
...
// deployNode deploys a new Popcateum node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gpop --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.popcateum"],
