...
// importGenesis imports a Gpop genesis spec into puppeth.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		// Export the native genesis spec used by puppeth and Gpop
		gpopJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile(gpopJson, out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gpopJson)

		// Export the genesis spec used by Aleth (formerly C++ Popcateum)
		...
		// Export the genesis spec used by Harmony (formerly PopcateumJ)
