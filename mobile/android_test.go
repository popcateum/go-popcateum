...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gpop.LightScryptN, Gpop.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.popcateum", new NodeConfig());
			node.start();
			...
			PopcateumClient ec = node.getPopcateumClient();
	...
	// signatures too. Regression test for go-popcateum issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gpop-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/popcateum/gpoptest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gpop and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.popcateum", "github.com/popcateum/go-popcateum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gpop.aar"), "gpop.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "popcateum", "gpoptest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.popcateum.gpoptest"
